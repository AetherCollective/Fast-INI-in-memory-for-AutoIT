...
ConsoleWrite( '-> $INI_memory = _IniAddSection($INI_memory, _IniInternalEnumSections($Ini_section)[0], $Ini_section)' & @CRLF)
$INI_memory = _IniAddSection($INI_memory, _IniInternalEnumSections($Ini_section)[0], $Ini_section)
$err = @error
ConsoleWrite( "$INI_memory: " & @CRLF & _IniGetMemory( $INI_memory ) & @CRLF)

ConsoleWrite( '-> _IniGetValue($INI_memory, _IniInternalEnumSections($Ini_section)[0], _IniInternalEnumSectionKeys($Ini_section)[0], ":-(")' & @CRLF)
